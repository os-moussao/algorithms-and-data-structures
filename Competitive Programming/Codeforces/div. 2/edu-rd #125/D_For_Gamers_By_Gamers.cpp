...
void solve()
{
	...
	for (int c = 1; c <= C; c++) {
		if (cost[c]) {
			for (int cc = c * 2, f = 2; cc <= C; cc += c, f++) {
				ckmax(cost[cc], f * cost[c]);
			}
		}
	}
	// filling empthy places with the optimal value
	for (int c = 1; c <= C; c++) {
		ckmax(cost[c], cost[c-1]);
	}
	cin >> m;
	FOR (i, m) {
		int d, h;
		cin >> d >> h;
		int ans = upper_bound(cost, cost + C + 1, d*h) - cost;
		cout << (ans != C + 1? ans: -1) << " ";
	}
}
