...
void solve()
{
	...
	for (int i = 0; i + 1 < s.L; i += 2) {
		...
		if (s[i] != s[i + 1]) {
			...
			if (i > 0) before = s[i - 1];
			if (i + 2 < s.L && s[i+2] == s[i+3]) after = s[i + 2];
			if (before == -1 && after == -1) {
				auto it = lower_bound(ALL(pos), i);
				if (it == pos.end()) s[i] = s[i+1];
				else {
					s[i] = s[i+1] = s[*it];
				}
			}
			else if (before == -1) s[i] = s[i + 1] = after;
			else if (after == -1) s[i] = s[i + 1] = before;
			else  s[i] = s[i + 1] = before;
		}
	}
	// debug(s);
	int mn = 0;
	for (int i = 0; i + 1 < s.L; i ++) {
		if (s[i] != s[i + 1]) mn++;
	}
	cout << ans << " " << mn+1 << endl;
}
