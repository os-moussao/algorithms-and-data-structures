...
void solve()
{
	...
	int max_need = -1;
	for (char c: s) ckmax(max_need, cneed(c));
	if (k >= max_need) {
		for (char &c: s) c = 'a';
		cout << s << endl;
		return ;
	...
	size_t last = 0;
	while (k) {
		size_t pos = s.find_first_not_of('a', last);
		if (pos == string::npos) {
			cout << s << endl;
			return ;
		}
		last = pos;
		int need = -1;
		for (int i = pos; i < s.L; i++) {
			if (need < cneed(s[i]) && cneed(s[i]) <= k) {
				need = cneed(s[i]);
			}
			else if (cneed)
		}
		int need = min(k, (int)(s[pos]-'a'));
		char tmp = s[pos];
		for (int i = pos; i < s.L; i++) {
			if (s[i] <= tmp && s[i] >= tmp - need) {
				s[i] = tmp - need;
			}
		}
		k -= need;
